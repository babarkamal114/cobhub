...
const Loader: React.FC<LoaderProps> = React.memo(
  ({ size, classNames, thickness: thicknessSize, color, ...props }) => {
    const strokeWidth = useMemo(
      () => (thicknessSize ? thickness[thicknessSize] : thickness.md),
      [thicknessSize]
    );

    return (
      <span
        className={cn(loaderVariants({ color, size }), classNames)}
        role="status"
        aria-label="Loading"
        {...props}
      >
